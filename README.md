...
`atspm` is a cutting-edge, lightweight Python package that transforms and aggregates raw traffic signal controller event logs into meaningful Aggregate Traffic Signal Performance Measures (ATSPMs). These measures help transportation agencies continuously monitor and optimize signal timing performance, detect issues, and take proactive measures-all in real-time. Currently used in production by the Oregon Department of Transportation (ODOT) through the Kinetic Signals platform, `atspm` simplifies the process of managing large datasets of traffic signal events, enabling more effective traffic management.
...
The Kinetic Signals platform is used to report ATSPMs which include various key parameters such as ***purdue phase termination***, ***split monitor***, ***pedestrian delay***, ***preemption details***, ***turning movement counts***, ***approach volume and delay***, ***arrivals on red***, ***yellow and red actuations***, ***purdue split failure***.
